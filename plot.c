...
int main(int argc,char *argv[])
{
  ...
  //Andras: read in contour separately
  FILE *my_contour_file;
  /*if (argc<3)
    {
        printf("Usage: iceplot gfile contourfile\n");
        exit(1);
    }
    this code does something sketchy with the argvs so i cant read it from there
    I think it processess all argvs so I have no place left to read this filename in
    oh well just copy the current to the contour_current file
    */
  my_contour_file = fopen("contour_current.txt","rt");
  Number_of_pinningsites = 0;
  if (my_contour_file!=NULL)
  	{
    fscanf(my_contour_file,"%d\n",&Number_of_pinningsites);
    pin_sites = (struct pinning_site *)malloc(Number_of_pinningsites*sizeof(struct pinning_site));
  	for(i=0;i<Number_of_pinningsites;i++)
  		{
  		fscanf(my_contour_file,"%e\n",&pin_sites[i].x);
  		fscanf(my_contour_file,"%e\n",&pin_sites[i].y);
  		fscanf(my_contour_file,"%e\n",&pin_sites[i].Ap);
  		fscanf(my_contour_file,"%e\n",&pin_sites[i].radiusx);
  		fscanf(my_contour_file,"%e\n",&pin_sites[i].radiusy);
  		}
  	printf("Read in %d pinningsites from %s\n",Number_of_pinningsites,"contour_current.txt");
    fflush(stdout);
  	fclose(my_contour_file);
  	}
	       ...
	       float syssizey,float xshift,float yshift,int do_net_contour)
{
  ...
  for(layr=(num_layers*num_nodes-1);layr>=0;layr--){
    ...
    //PARTICLES
    //Andras: I doubled this for cycle - 
    //to draw the vertices first and then the particles
    //also the pinning sites need to be drawn between these
    //what overlaps what considerations

    //PLOT THE VERTICES
    for (i=0; i<num_pars; i++) 
    	if (!((cmframes[i].color==2)||(cmframes[i].color==3)||(cmframes[i].color==4)))
    {
...
//printf("%d\n",Number_of_pinningsites);
  ...
  for(i=0;i<Number_of_pinningsites;i++)
	...
	Box2 = x_scale *7.0; Box = Box2/2;

  	XSetForeground(display,pixmap_gc,(BlackPixel(display,screen_num)));
    
    //old version has i%2 that is when pinning sites were not missing
    //new version: what I have in the contour file as radiusy is actually the cosfi of the pinning site
    	if (pin_sites[i].radiusy==0) XDrawArc(display,pixmap,pixmap_gc,par_x-Box,par_y-3*Box,Box2,Box2*3,0,360*64);
	else XDrawArc(display,pixmap,pixmap_gc,par_x-3*Box,par_y-Box,Box2*3,Box2,0,360*64);
...
/*  
  if ((color==2)||(color==3))	{Box2 = x_scale*3/5*1.0; Box = Box2/2;} //particles
  else 				{Box2 = x_scale*8/5*1.0; Box = Box2/2;}	//vertices
*/

  Box2 = x_scale*10.0/5*1.0; Box = Box2/2;
  ...
  {
    //actually drawing the particle and the vertex
    ...
    if (color==2) {
  //drawing the arrow inside the particle for active matter
  //theta is the cump_disp actually
  //we will send the angle of the motor force here
  //for active particles

  arrow_points[0].x = Box/3.0;
  arrow_points[0].y = Box/3.0;
  arrow_points[1].x = Box/3.0;
  arrow_points[1].y = Box*2.0/3.0;
  arrow_points[2].x = Box;
  arrow_points[2].y = 0;
  arrow_points[3].x = Box/3.0;
  arrow_points[3].y = -Box*2.0/3.0-1;
  arrow_points[4].x = Box/3.0;
  arrow_points[4].y = -Box/3.0-1;
  arrow_points[5].x = -Box*2.0/3.0;
  arrow_points[5].y = -Box/3.0-1;
  arrow_points[6].x = -Box*2.0/3.0;
  arrow_points[6].y = Box/3.0;
  arrow_points[7].x = Box/3.0;
  arrow_points[7].y = Box/3.0;
  ...
  for(char i=0;i<8;i++)
    {
      //rotate arrow  
      short xnew =  100.0*arrow_points[i].x*cos(-theta) - 100.0*arrow_points[i].y*sin(-theta);
      short ynew =  100.0*arrow_points[i].x*sin(-theta) + 100.0*arrow_points[i].y*cos(-theta);
      
      //place it on the particle
      arrow_points[i].x = par_x + xnew/100.0;
      arrow_points[i].y = par_y + ynew/100.0;
    }

  XSetForeground(display,pixmap_gc,(WhitePixel(display,screen_num)));
  XDrawLines(display,  pixmap, pixmap_gc, arrow_points, 8, CoordModeOrigin);
  }
			  ...
			  float *PinSize)
{
  ...
  if(get_arg(line,&pos,filename)){
    ...
    *do_S_contour = 1;
    ...
    Number_of_pinningsites = num_pins;
	     ...
	     int tripB[],int tripC[],int *num_trip)
{
  ...
  void endpoint(struct Edge *e,int lr,struct Site *s,int cjoleft[],
...
/*=================== endpoint ==================*/
void endpoint(struct Edge *e,int lr,struct Site *s,int cjoleft[],int cjoright[],
	 int *num_ep)
{
  e -> ep[lr] = s;
  ref(s);
