...
// Copyright @Levente Varga 2023 Oct - Simulations and Modeling Group
...
struct global_struct
{
    ...
    // active grid changes
    double active_pheromon_absorption;
    double active_pheromon_recovery;
...
void calculate_active_grid_absorption();
...
void initialize_from_file(int argc, char *argv[])
{
    ...
    read_line_from_file(parameterfile, "active_pheromon_absorption", "double", &global.active_pheromon_absorption);
    read_line_from_file(parameterfile, "active_pheromon_recovery", "double", &global.active_pheromon_recovery);
...
#else
    if (global.seedToSet == 0)
        sprintf(global.baseName, "s%dN%d_p_perc%.1lf_Tsteps%d_dt%lf_actDiv%lf_actAbs%lf_actRec%lf", dist_type, global.N_particles, global.passive_particle_percentage, global.total_runtime, global.dt, global.active_grid_division, global.active_pheromon_absorption, global.active_pheromon_recovery);
    else
        sprintf(global.baseName, "seeds/seed_%ld_s%dN%d_p_perc%.1lf_Tsteps%d_dt%lf_actDiv%lf_actAbs%lf_actRec%lf", global.seedToSet, dist_type, global.N_particles, global.passive_particle_percentage, global.total_runtime, global.dt, global.active_grid_division, global.active_pheromon_absorption, global.active_pheromon_recovery);
#endif
...
{
    if (x >= x0 && x <= x0 + width / 2)
        return y0 + 2 * height / width * (x - x0);
    else if (x > x0 + width / 2 && x <= x0 + width)
        return y0 - 2 * height / width * (x - x0 - width);
}
...
// covering only half the area with pinning sites
    ...
    for (i = 0; i < global.N_pinningsites; i++)
    {
        ...
        while (overlap)
        {
            ...
            if (Rand() < 0.5)
                y_temp = global.SY / 2. * arbitrary_distribution(Rand());
            else
                y_temp = global.SY / 2. * arbitrary_distribution(Rand()) + global.SY / 2.;
#elif dist_type == -1
            ...
            if (global.particle_x[i] < center_grid_x)
            {
                if (global.particle_y[i] < center_grid_y)
                {
                    for (k = 0; k < global.active_grid_neighbours; k++)
                    {
                        ...
                        // PBC foldback
                        if (act_i < 0)
                            act_i = global.active_grid_Nx + act_i;
                        else if (act_i >= global.active_grid_Nx)
                            act_i = act_i - global.active_grid_Nx;
                        if (act_j < 0)
                            act_j = global.active_grid_Ny + act_j;
                        else if (act_j >= global.active_grid_Ny)
                            act_j = act_j - global.active_grid_Ny;
                        ...
                        // recovery
                        global.active_grid[act_i][act_j] += global.active_pheromon_recovery;
                        if (global.active_grid[act_i][act_j] > 1)
                        {
                            global.active_grid[act_i][act_j] = 1;
                        }
                    }
                }
                else
...
void calculate_active_grid_absorption()
{
    ...
    for (i = 0; i < global.active_grid_Nx; i++)
    {
        for (j = 0; j < global.active_grid_Ny; j++)
        {
            ...
            // absorption
            global.active_grid[i][j] -= global.active_pheromon_absorption;
            if (global.active_grid[i][j] < 0)
            {
                global.active_grid[i][j] = 0;
            }
        }
...
void run_simulation()
{
    ...
    for (global.time = 0; global.time < global.total_runtime; global.time++)
    {
        ...
        calculate_active_grid_absorption();
